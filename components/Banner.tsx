...
const Banner = ({ netflixOriginals }: IProps) => {
  ...
  return (
    <div
      ...
      <h1
        className='text-2xl lg:text-7xl md:text-4xl font-bold text-shadow-md'>{movie?.title || movie?.name || movie?.original_name}</h1>
      <p
        className='max-w-xs text-xs md:max-w-lg md:text-lg lg:max-w-2xl lg:text-2xl text-shadow-md'>{movie?.overview}</p>
